                    ...
                    <div className='text-slate-600' onClick={handleShowMenu}>
                        ...
                        </div>
                        {
                            showMenu && <div className='absolute right-2 bg-white py-2 shadow drop-shadow-md flex flex-col min-w-[120px] text-center'>
                                {
                                    userData.email === process.env.REACT_APP_ADMIN_EMAIL && <Link to={'new-product'} className='whitespace-nowrap cursor-pointer px-2'>New Product</Link>
                                }

                                {
                                    userData.image ? <p className='cursor-pointer px-2 text-white bg-red-500' onClick={handleLogout}>Logout ({userData.firstName})</p> : <Link to={'login'} className='whitespace-nowrap cursor-pointer  px-2'>Login</Link>
                                }
                                <nav className='text-base md:text-lg flex flex-col md:hidden'>
                                    <Link to={"/"} className='px-2 py-1'>Home</Link>
                                    <Link to={"menu/64762133d9105266f507cb36"} className='px-2 py-1'>Menu</Link>
